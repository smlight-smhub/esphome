...
extern "C" void smhub_ipc_pref_load_cb(uint32_t hash, const uint8_t *data, size_t len) {
    if (hash == pref_load_target_hash) {
        ...
        ESP_LOGV(TAG, "CB Received pref_load_resp for hash %lu, len %d, data: %02X", (unsigned long)hash, (int)len, len > 0 ? data[0] : 0);
    }
}
...
bool SG2000PreferenceBackend::save(const uint8_t *data, size_t len) {
    ...
    ESP_LOGV(TAG, "Saving preference hash %lu, len %d, data: %02X", (unsigned long)this->type_, (int)len, len > 0 ? data[0] : 0);
...
bool SG2000PreferenceBackend::load(uint8_t *data, size_t len) {
    ...
    ESP_LOGV(TAG, "Preference loaded successfully for hash %lu, data: %02X", (unsigned long)this->type_, len > 0 ? data[0] : 0);
    return true;
}

void setup_preferences() {
    global_preferences = new SG2000Preferences();
}
