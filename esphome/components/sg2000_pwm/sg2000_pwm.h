...
class Sg2000PWM : public output::FloatOutput, public Component {
 public:
  ...
  void set_pin_mux(uint32_t reg, uint32_t func) { pin_reg_ = reg; pin_func_ = func; }

  void setup() override {
    ESP_LOGI(TAG, "Setting up SG2000 PWM%u Channel %u at %.1f Hz...", pwm_id_, channel_, frequency_);
    uint32_t req_mask = 0;
    if (pwm_id_ == 0) req_mask = sg2000::SMHUB_HW_PWM0;
    else if (pwm_id_ == 1) req_mask = sg2000::SMHUB_HW_PWM1;
    else if (pwm_id_ == 2) req_mask = sg2000::SMHUB_HW_PWM2;
    else if (pwm_id_ == 3) req_mask = sg2000::SMHUB_HW_PWM3;
